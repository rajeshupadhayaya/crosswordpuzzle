//array which store the x,y and char value which will be used for validation purpose.
var stored_chars = {};
//var store_values = {};
var score = 0;
            ...
            for(var c = 0; c < grid[r].length; c++){
                ...
                for(var k in groups){
                    ...
                    if(cell && cell[k] && cell[k]['is_start_of_word']){
						...
						// console.log('clues ' + clues_in[index]);
						// console.log('words ' + words_in[index]);
                    }
                }
    ...
    var minimizeGrid = function(){
        ...
        for(var r = r_min, r2 = 0; r2 < rows; r++, r2++){
            for(var c = c_min, c2 = 0; c2 < cols; c++, c2++){
				console.log('min row ' +r2 + 'min col ' + c2);
        ...
        for(var r = 0; r < grid.length; r++){
            ...
            for(var c = 0; c < grid[r].length; c++){
                ...
                if(cell == null){
                    ...
                    var char = cell['char'];
                    var store_values = [r + ',' + c,char];
                    // var store_values[name] = char;
                    name = r + ',' + c;
                    store_data(name, char) ;
                    var css_class = "all-border";
                    var is_start_of_word = (cell['across'] && cell['across']['is_start_of_word']) || (cell['down'] && cell['down']['is_start_of_word']);
                    var input_box = "<input id='" + r + "," + c + "' name='" + r + "," + c + "' class='input-box' autocomplete='off' type='text' size='1' maxlength='1' value=''>";
                }

                if(is_start_of_word) {
                    var img_url = CrosswordUtils.PATH_TO_PNGS_OF_NUMBERS + label + ".png";
                    html.push("<td class='" + css_class + "' style=\"background-image:url('" + img_url + "');background-repeat:no-repeat\">" + input_box);
                    label++;
                } else {
                    html.push("<td class='" + css_class + "'>" + input_box);
                }
...
var store_data = function(name, char){
    stored_chars[name] = char ;
} 
 

/*var check_result = function(stored_chars){
